...
------------------------------------------------------------------------------------------------------------------------------
DROP MATERIALIZED VIEW IF EXISTS visufoncier.v2_insee_temporel_view CASCADE;

CREATE MATERIALIZED VIEW visufoncier.v2_insee_temporel_view AS
SELECT 
    i.insee_com,
    i.nom_commune,
    c.siren_epci,
    ...
    ff.scot AS nom_scot,
    m.annee_valeur,
    make_timestamp(m.annee_valeur, 1, 1, 0, 0, 0) AS annee,
    -- Population
    CASE m.annee_valeur
        WHEN 2011 THEN i.pnum2011 WHEN 2012 THEN i.pnum2012 WHEN 2013 THEN i.pnum2013
        WHEN 2014 THEN i.pnum2014 WHEN 2015 THEN i.pnum2015 WHEN 2016 THEN i.pnum2016
        WHEN 2017 THEN i.pnum2017 WHEN 2018 THEN i.pnum2018 WHEN 2019 THEN i.pnum2019
        WHEN 2020 THEN i.pnum2020 WHEN 2021 THEN i.pnum2021 WHEN 2022 THEN i.pnum2022
        WHEN 2023 THEN i.pnum2023
    END::numeric AS population,
    -- Emploi
    CASE m.annee_valeur
        WHEN 2011 THEN i.p11_emplt WHEN 2017 THEN i.p17_emplt WHEN 2018 THEN i.p18_emplt
        WHEN 2020 THEN i.p20_emplt WHEN 2022 THEN i.p22_emplt
    END::numeric AS emploi,
    -- Ménages
    CASE m.annee_valeur
        WHEN 2011 THEN i.c11_men WHEN 2017 THEN i.c17_men WHEN 2018 THEN i.c18_men
        WHEN 2019 THEN i.c19_men WHEN 2020 THEN i.c20_men WHEN 2022 THEN i.c22_men
    END::numeric AS menages
...
LEFT JOIN ign.express_commune c ON i.insee_com = c.insee_com
LEFT JOIN ign.express_epci e ON c.siren_epci = e.code_epci
LEFT JOIN visufoncier.ff_obs_artif_conso_com_2009_2024 ff ON i.insee_com = ff.idcom;

CREATE INDEX idx_insee_temp_com ON visufoncier.v2_insee_temporel_view (insee_com);
CREATE INDEX idx_insee_temp_annee ON visufoncier.v2_insee_temporel_view (annee_valeur);
CREATE INDEX idx_insee_temp_epci ON visufoncier.v2_insee_temporel_view (nom_epci);
...
-- =====================================================
DROP MATERIALIZED VIEW IF EXISTS visufoncier.v2_insee_evolution_view CASCADE;

CREATE MATERIALIZED VIEW visufoncier.v2_insee_evolution_view AS
WITH periodes AS (
    SELECT 2011 AS annee_debut, 2021 AS annee_fin
    UNION ALL SELECT 2021, 2024
    UNION ALL SELECT 2011, 2024
),
surfaces AS (
    SELECT 
        insee_com,
        EXTRACT(YEAR FROM annee)::integer AS annee_valeur,
        SUM(CASE WHEN enaf_conso = 'conso' THEN surface_ha ELSE 0 END)::numeric AS surf_conso_ha
    FROM visufoncier.v2_mos_agrege
    GROUP BY insee_com, EXTRACT(YEAR FROM annee)::integer
)
SELECT 
    i.insee_com,
    i.nom_commune,
    ...
    END AS nom_departement,
    ff.scot AS nom_scot,
    make_timestamp(p.annee_debut, 1, 1, 0, 0, 0) AS annee_debut,
    make_timestamp(p.annee_fin, 1, 1, 0, 0, 0) AS annee_fin,
    p.annee_debut AS annee_debut_valeur,
    p.annee_fin AS annee_fin_valeur,
    
    -- Population
    CASE p.annee_debut WHEN 2011 THEN i.pnum2011 WHEN 2021 THEN i.pnum2021 END::numeric AS pop_debut,
    CASE p.annee_fin WHEN 2021 THEN i.pnum2021 WHEN 2024 THEN i.pnum2023 END::numeric AS pop_fin,
    (CASE p.annee_fin WHEN 2021 THEN i.pnum2021 WHEN 2024 THEN i.pnum2023 END -
     CASE p.annee_debut WHEN 2011 THEN i.pnum2011 WHEN 2021 THEN i.pnum2021 END)::numeric AS pop_diff,
    CASE WHEN CASE p.annee_debut WHEN 2011 THEN i.pnum2011 WHEN 2021 THEN i.pnum2021 END > 0 THEN
        ROUND(((CASE p.annee_fin WHEN 2021 THEN i.pnum2021 WHEN 2024 THEN i.pnum2023 END::numeric -
                CASE p.annee_debut WHEN 2011 THEN i.pnum2011 WHEN 2021 THEN i.pnum2021 END::numeric) /
               CASE p.annee_debut WHEN 2011 THEN i.pnum2011 WHEN 2021 THEN i.pnum2021 END::numeric) * 100, 1)
    END AS pop_taux_evol,
    
    -- Emploi
    CASE p.annee_debut WHEN 2011 THEN i.p11_emplt WHEN 2021 THEN i.p20_emplt END::numeric AS emploi_debut,
    CASE p.annee_fin WHEN 2021 THEN i.p20_emplt WHEN 2024 THEN i.p22_emplt END::numeric AS emploi_fin,
    (CASE p.annee_fin WHEN 2021 THEN i.p20_emplt WHEN 2024 THEN i.p22_emplt END -
     CASE p.annee_debut WHEN 2011 THEN i.p11_emplt WHEN 2021 THEN i.p20_emplt END)::numeric AS emploi_diff,
    CASE WHEN CASE p.annee_debut WHEN 2011 THEN i.p11_emplt WHEN 2021 THEN i.p20_emplt END > 0 THEN
        ROUND(((CASE p.annee_fin WHEN 2021 THEN i.p20_emplt WHEN 2024 THEN i.p22_emplt END::numeric -
                CASE p.annee_debut WHEN 2011 THEN i.p11_emplt WHEN 2021 THEN i.p20_emplt END::numeric) /
               CASE p.annee_debut WHEN 2011 THEN i.p11_emplt WHEN 2021 THEN i.p20_emplt END::numeric) * 100, 1)
    END AS emploi_taux_evol,
    
    -- Ménages
    CASE p.annee_debut WHEN 2011 THEN i.c11_men WHEN 2021 THEN i.c20_men END::numeric AS menages_debut,
    CASE p.annee_fin WHEN 2021 THEN i.c20_men WHEN 2024 THEN i.c22_men END::numeric AS menages_fin,
    (CASE p.annee_fin WHEN 2021 THEN i.c20_men WHEN 2024 THEN i.c22_men END -
     CASE p.annee_debut WHEN 2011 THEN i.c11_men WHEN 2021 THEN i.c20_men END)::numeric AS menages_diff,
    CASE WHEN CASE p.annee_debut WHEN 2011 THEN i.c11_men WHEN 2021 THEN i.c20_men END > 0 THEN
        ROUND(((CASE p.annee_fin WHEN 2021 THEN i.c20_men WHEN 2024 THEN i.c22_men END::numeric -
                CASE p.annee_debut WHEN 2011 THEN i.c11_men WHEN 2021 THEN i.c20_men END::numeric) /
               CASE p.annee_debut WHEN 2011 THEN i.c11_men WHEN 2021 THEN i.c20_men END::numeric) * 100, 1)
    ...
    -- Densité début
    CASE WHEN sd.surf_conso_ha > 0 THEN
        ROUND((COALESCE(CASE p.annee_debut WHEN 2011 THEN i.pnum2011 WHEN 2021 THEN i.pnum2021 END, 0) +
               COALESCE(CASE p.annee_debut WHEN 2011 THEN i.p11_emplt WHEN 2021 THEN i.p20_emplt END, 0))::numeric 
              / sd.surf_conso_ha, 2)
    END AS densite_debut,
    
    -- Densité fin
    CASE WHEN sf.surf_conso_ha > 0 THEN
        ROUND((COALESCE(CASE p.annee_fin WHEN 2021 THEN i.pnum2021 WHEN 2024 THEN i.pnum2023 END, 0) +
               COALESCE(CASE p.annee_fin WHEN 2021 THEN i.p20_emplt WHEN 2024 THEN i.p22_emplt END, 0))::numeric 
              / sf.surf_conso_ha, 2)
    END AS densite_fin,
    
    -- Différence densité
    CASE WHEN sd.surf_conso_ha > 0 AND sf.surf_conso_ha > 0 THEN
        ROUND((COALESCE(CASE p.annee_fin WHEN 2021 THEN i.pnum2021 WHEN 2024 THEN i.pnum2023 END, 0) +
               COALESCE(CASE p.annee_fin WHEN 2021 THEN i.p20_emplt WHEN 2024 THEN i.p22_emplt END, 0))::numeric 
              / sf.surf_conso_ha, 2) -
        ROUND((COALESCE(CASE p.annee_debut WHEN 2011 THEN i.pnum2011 WHEN 2021 THEN i.pnum2021 END, 0) +
    ...
    -- Taux évolution densité (%)
    CASE WHEN sd.surf_conso_ha > 0 AND sf.surf_conso_ha > 0 AND
              (COALESCE(CASE p.annee_debut WHEN 2011 THEN i.pnum2011 WHEN 2021 THEN i.pnum2021 END, 0) +
               COALESCE(CASE p.annee_debut WHEN 2011 THEN i.p11_emplt WHEN 2021 THEN i.p20_emplt END, 0)) > 0 THEN
        ROUND((
            ((COALESCE(CASE p.annee_fin WHEN 2021 THEN i.pnum2021 WHEN 2024 THEN i.pnum2023 END, 0) +
              COALESCE(CASE p.annee_fin WHEN 2021 THEN i.p20_emplt WHEN 2024 THEN i.p22_emplt END, 0))::numeric / sf.surf_conso_ha) -
            ((COALESCE(CASE p.annee_debut WHEN 2011 THEN i.pnum2011 WHEN 2021 THEN i.pnum2021 END, 0) +
              COALESCE(CASE p.annee_debut WHEN 2011 THEN i.p11_emplt WHEN 2021 THEN i.p20_emplt END, 0))::numeric / sd.surf_conso_ha)
        ) / ((COALESCE(CASE p.annee_debut WHEN 2011 THEN i.pnum2011 WHEN 2021 THEN i.pnum2021 END, 0) +
              COALESCE(CASE p.annee_debut WHEN 2011 THEN i.p11_emplt WHEN 2021 THEN i.p20_emplt END, 0))::numeric / sd.surf_conso_ha) * 100, 1)
...
CREATE INDEX idx_insee_evol_com ON visufoncier.v2_insee_evolution_view (insee_com);
CREATE INDEX idx_insee_evol_annees ON visufoncier.v2_insee_evolution_view (annee_debut_valeur, annee_fin_valeur);
CREATE INDEX idx_insee_evol_epci ON visufoncier.v2_insee_evolution_view (nom_epci);
...
---MOS X GPU 
-- =====================================================
DROP MATERIALIZED VIEW IF EXISTS visufoncier.v2_mos_foncier_agrege_gpu_view CASCADE;
...
AS WITH communes AS (
    ...
    LEFT JOIN ign.express_epci e ON c.siren_epci = e.code_epci
    LEFT JOIN visufoncier.ff_obs_artif_conso_com_2009_2024 ff ON c.insee_com = ff.idcom
    WHERE c.insee_dep IN ('22', '29', '35', '56')
), calc AS (
    SELECT 
        insee_com,
        COALESCE(st_area(st_union(geom)) / 10000.0, 0) AS surface_total,
        COALESCE(st_area(st_union(CASE WHEN typezone = 'U' THEN geom END)) / 10000.0, 0) AS surface_u,
        COALESCE(st_area(st_union(CASE WHEN typezone = 'AUc' THEN geom END)) / 10000.0, 0) AS surface_auc,
        COALESCE(st_area(st_union(CASE WHEN typezone = 'AUs' THEN geom END)) / 10000.0, 0) AS surface_aus
    FROM visufoncier.v2_gpu_mos_enaf_plu_plui
    GROUP BY insee_com
), secteurcc AS (
    SELECT 
        insee_com,
        COALESCE(sum(CASE WHEN typesect = '01' THEN st_area(geom) / 10000.0 ELSE 0 END), 0) AS surface_secteurcc_01,
        COALESCE(sum(CASE WHEN typesect = '02' THEN st_area(geom) / 10000.0 ELSE 0 END), 0) AS surface_secteurcc_02,
        COALESCE(sum(st_area(geom) / 10000.0), 0) AS surface_secteurcc_total
    FROM visufoncier.v2_gpu_mos_enaf_cc
    GROUP BY insee_com
), calc_ajust AS (
    SELECT 
        insee_com,
        COALESCE(st_area(st_union(geom)) / 10000.0, 0) AS surface_total_ajust,
        COALESCE(st_area(st_union(CASE WHEN typezone = 'U' THEN geom END)) / 10000.0, 0) AS surface_u_ajust,
        COALESCE(st_area(st_union(CASE WHEN typezone = 'AUc' THEN geom END)) / 10000.0, 0) AS surface_auc_ajust,
        COALESCE(st_area(st_union(CASE WHEN typezone = 'AUs' THEN geom END)) / 10000.0, 0) AS surface_aus_ajust
    FROM visufoncier.v2_gpu_mos_enaf_plu_plui_ajust
    GROUP BY insee_com
), secteurcc_ajust AS (
    SELECT 
        insee_com,
        COALESCE(sum(CASE WHEN typesect = '01' THEN st_area(geom) / 10000.0 ELSE 0 END), 0) AS surface_secteurcc_01_ajust,
        COALESCE(sum(CASE WHEN typesect = '02' THEN st_area(geom) / 10000.0 ELSE 0 END), 0) AS surface_secteurcc_02_ajust,
        COALESCE(sum(st_area(geom) / 10000.0), 0) AS surface_secteurcc_total_ajust
...
SELECT 
    ...
    a.insee_dep,
    a.nom_departement,
    COALESCE(calc.surface_total, 0) AS surface_total,
    COALESCE(calc.surface_u, 0) AS surface_u,
    COALESCE(calc.surface_auc, 0) AS surface_auc,
    COALESCE(calc.surface_aus, 0) AS surface_aus,
    COALESCE(secteurcc.surface_secteurcc_01, 0) AS surface_secteurcc_01,
    COALESCE(secteurcc.surface_secteurcc_02, 0) AS surface_secteurcc_02,
    COALESCE(secteurcc.surface_secteurcc_total, 0) AS surface_secteurcc_total,
    COALESCE(calc_ajust.surface_total_ajust, 0) AS surface_total_ajust,
    COALESCE(calc_ajust.surface_u_ajust, 0) AS surface_u_ajust,
    COALESCE(calc_ajust.surface_auc_ajust, 0) AS surface_auc_ajust,
    COALESCE(calc_ajust.surface_aus_ajust, 0) AS surface_aus_ajust,
    COALESCE(secteurcc_ajust.surface_secteurcc_01_ajust, 0) AS surface_secteurcc_01_ajust,
    COALESCE(secteurcc_ajust.surface_secteurcc_02_ajust, 0) AS surface_secteurcc_02_ajust,
    COALESCE(secteurcc_ajust.surface_secteurcc_total_ajust, 0) AS surface_secteurcc_total_ajust
FROM communes a
LEFT JOIN calc ON a.insee_com = calc.insee_com
LEFT JOIN secteurcc ON a.insee_com = secteurcc.insee_com
LEFT JOIN calc_ajust ON a.insee_com = calc_ajust.insee_com
LEFT JOIN secteurcc_ajust ON a.insee_com = secteurcc_ajust.insee_com
WITH DATA;

CREATE INDEX idx_v2_gpu_insee ON visufoncier.v2_mos_foncier_agrege_gpu_view (insee_com);
CREATE INDEX idx_v2_gpu_epci ON visufoncier.v2_mos_foncier_agrege_gpu_view (nom_epci);
CREATE INDEX idx_v2_gpu_scot ON visufoncier.v2_mos_foncier_agrege_gpu_view (nom_scot);
